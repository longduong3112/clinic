<?php 
	...
	$p = new Mclass;
	$username = $password = $fullname = $sdt = $email = $fname = $lname = $address = $image ='';
	$error = ['username' => '', 'password' => '', 'fullname' => '', 'sdt' => '', 'email' => '', 'fname' => '', 'lname' => '', 'address' => '', 'image' => ''];
	if(isset($_POST['signup'])):
		...
		if(!empty($_POST['fname'])):
			$fname = mysqli_real_escape_string($p->connDB(), $_POST['fname']);
		else:
			$error['fname'] = 'Không được để trống.';	
		endif;
		//kiểm tra lname
		if(!empty($_POST['lname'])):
			$lname = mysqli_real_escape_string($p->connDB(), $_POST['lname']);
		else:
			$error['lname'] = 'Không được để trống.';	
		endif;
		//kiểm tra sdt
		if(!empty($_POST['sdt'])):
		...
		if(!array_filter($error)):
			...
			$sql = "insert into user(unique_id, username, password, fullname, address, sdt, email, fname, lname, image)
			values({$ran_id}, '$username', '$password', '$fullname', '$address', '$sdt', '$email', '$fname', '$lname', '$image')";
	...
	<div class="row">
		...
		<div class="col">
			<form action="signup.php" class="card p-3" method="POST">
			  ...
			  <div class="form-group">
			    <label class="font-weight-bolder" style="font-size:17px">Fname: </label><span class="text-danger"><?php echo $error['fname']; ?></span>
			    <input type="text" class="form-control" name="fname" value="<?php echo $fname; ?>">
			  </div>
			  <div class="form-group">
			    <label class="font-weight-bolder" style="font-size:17px">Lname: </label><span class="text-danger"><?php echo $error['lname']; ?></span>
			  ...
			  <label class="font-weight-bolder" style="font-size:17px">Chọn ảnh đại diện: </label><span class="text-danger"><?php echo $error['image']; ?></span>
